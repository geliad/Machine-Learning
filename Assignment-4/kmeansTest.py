...
def transform_image(image, code_vectors):
    ...
    squared_euclidean_distances = np.zeros([image.shape[0], image.shape[1], len(code_vectors)])

    # Compute squared euclidean distances for each k
    for k in range(len(code_vectors)):
        euclidean_distances = np.linalg.norm(code_vectors[k] - image[:, :, :], axis=2)
        squared_euclidean_distances[:, :, k] = np.power(euclidean_distances, 2)

    membership = np.argmin(squared_euclidean_distances, axis=2)

    new_image = code_vectors[membership]
    return new_image
